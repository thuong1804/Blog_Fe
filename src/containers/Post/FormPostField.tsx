...
import { AuthorPageProps, FormValuesPost, optionProps, OptionType } from "@/type/typeProps";
        ...
        <div className="modal-box bg-white  max-w-4xl h-[90%]">
          ...
          <div className="mt-4">
            <div className="w-1/4 h-[200px] rounded-2xl bg-gray-600 flex items-center justify-center gap-2 relative">
              {file ? (
                <Image src={file} alt="thumbnail" fill className="object-cover rounded-2xl" />
              ) : (
                <> <IoIosCamera className="text-xl" /> Thumbnail</>
              )}
            </div>
            ...
            <form className="w-full mt-3 grid grid-cols-2 gap-5" onSubmit={handleSubmitForm} id="form-post">
              <SelectField
                title="Tags"
                required
            ...
            <button type="submit" form="close" className="btn btn-sm transition-transform duration-200 hover:border-1 hover:border-gray-200 text-black hover:bg-white btn-circle btn-ghost absolute right-2 top-2">✕</button>
            </form>
          </div>
          <div className="modal-action">
            <form method="dialog" className="flex items-center gap-3" id="close">
              <button className="btn bg-white text-black border-1 border-gray-400">Close</button>
              <button className="btn" form="form-post">Save</button>
            </form>
          </div>
        </div>
      </dialog>
